...
// This file is now simple, secure, and has no API keys.
import apiClient from "./axios"; // Import your configured axios client
...
export const getChatbotResponse = async (
  ...
  // 1. Create the payload that matches our Pydantic schema
  const payload = {
    task_context: taskContext || {}, // Send empty object if null
    chat_history: chatHistory || [],
    new_question: newUserQuestion,
  };

  try {
    // 2. Call our *own* backend, not Google's
    const response = await apiClient.post("/api/v1/chat/", payload);

    // 3. Return the text response from our server
    return response.data.text;
  } catch (error) {
    console.error("Error calling backend chat proxy:", error);
    // Pass the error message from the *backend* to the UI
    const errorMsg =
      error.response?.data?.detail || "The AI assistant is unavailable.";
