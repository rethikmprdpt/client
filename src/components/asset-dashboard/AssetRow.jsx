...
import {
  ...
  Trash2,
  Lock,
  RefreshCw, // --- NEW: Import Swap Icon ---
} from "lucide-react";

// --- AssetDetailView ---
// Shows expanded details and (conditionally) Edit/Delete/Swap buttons
function AssetDetailView({ asset, onEdit, onDelete, onSwap }) {
  let details = [];
  let locationInfo = null;
  const isAssigned = asset.status?.toLowerCase() === "assigned";

  // Check if this asset type supports swapping (only ONTs and Routers for now)
  const canSwap =
    isAssigned && (asset.type === "ONT" || asset.type === "Router");
    ...
    <div className="p-4 pt-4 mt-4 border-t border-gray-200">
      <div className="flex justify-between items-start">
        ...
        <div className="flex flex-col gap-2 w-40 shrink-0">
          {" "}
          {/* Slightly wider for Swap button */}
          {isAssigned ? (
            <div className="flex flex-col gap-2">
              {/* Locked Message */}
              <div className="flex items-center gap-2 text-left px-3 py-2 text-xs font-semibold text-gray-500 bg-gray-100 rounded-lg border border-gray-200">
              ...
              {canSwap && (
                <button
                  ...
                  className="flex items-center gap-2 w-full text-left px-3 py-2 text-sm font-semibold text-purple-600 bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors border border-purple-100"
                >
                  <RefreshCw size={16} />
                  Swap / RMA
                </button>
            ...
            // If not assigned, show Edit/Delete
            <>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className="flex items-center gap-2 w-full text-left px-3 py-2 text-sm font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors"
              >
                <Edit size={16} />
                Edit
              </button>
              <button
                ...
                className="flex items-center gap-2 w-full text-left px-3 py-2 text-sm font-semibold text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition-colors"
              >
                <Trash2 size={16} />
                Delete
              </button>
