...
const EditAssetModal = ({ isOpen, onClose, onSave, asset }) => {
  ...
  useEffect(() => {
    if (asset) {
      setFormData({
        model: asset.model || "",
        // We access originalData to get the pincode if it wasn't in the top-level object
        pincode: asset.originalData?.pincode || "",
        status: asset.status || "",
      });
    }
  }, [asset]);
  ...
  const handleSubmit = async (e) => {
    ...
    // Pass data back to parent to handle API call
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md m-4">
        ...
        <form onSubmit={handleSubmit} className="p-6">
            ...
            <div>
              ...
              {asset.status === "assigned" && (
                <p className="text-xs text-red-500 mt-1">
                  Note: Assigned assets cannot be edited. Unassign first.
                </p>
              )}
            </div>
          </div>
          ...
          <div className="mt-6 flex justify-end space-x-3">
            ...
            <button
              type="submit"
              disabled={isSubmitting || asset.status === "assigned"}
